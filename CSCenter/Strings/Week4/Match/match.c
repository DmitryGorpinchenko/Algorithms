...
void match(SuffixTree* st, char* pattern, int* is_starts);
void fill_starts(Node* subtree, int* starts);
...
int main() {
    ...
    int* is_starts = calloc(size, sizeof(int));
    for(int i = 0; i < n; i++) {
        match(st, pattern, is_starts);
...
void match(SuffixTree* st, char* pattern, int* is_starts) {
    ...
    fill_starts(cur, is_starts);
}

void fill_starts(Node* subtree, int* is_starts) {
    if(subtree == NULL) {
        return;
    }
    if(subtree->pos != -1) {
        is_starts[subtree->pos] = 1;
    }
    for(int i = 0; i < ALPHA_SIZE; i++) {
        fill_starts(subtree->nxt[i], is_starts);
    }
}
